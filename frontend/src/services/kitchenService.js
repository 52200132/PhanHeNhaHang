...
export const updateOrderStatus = async (kitchenOrderId, newStatus) => {
  try {
    const response = await axios.patch(`${API_URL}/kitchen_orders/${kitchenOrderId}`, {
      status: newStatus
    });
    return response.data;
  } catch (error) {
...
export const updateOrderServiceStatus = async (orderId, status) => {
  try {
    const response = await axios.patch(`${API_URL}/call-order-payment-service/orders/${orderId}`, {
      status
    });
    return response.data;
  } catch (error) {
